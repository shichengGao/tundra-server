...
int main(int argc, char* argv[]) {
    ...
    Logging::InitLoggingConfig("MainTest", Logging::rollSize_, Logging::flushInterval_);
    ...
    Logging::instance().log_warn("prepare to launch.");


    HttpServer server(&loop, listenAddr, "tundra-http-server", TcpServer::kReusePort);
    server.setHttpCallback(onRequest);
    server.setThreadNum(numThreads);

    Logging::instance().log_warn("now start http server.");
    server.start();
